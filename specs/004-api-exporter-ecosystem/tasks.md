...
- [ ] T041 [US1] Implement `GetRulesPagedAsync` in `src/FinancialOS.Data/EfFinancialRepository.cs`: filter by `RuleType` (case-insensitive), `IsEnabled`, `CategoryId`; order by `Priority` descending then `Id`; return `PagedResult<Rule>`
